//Use concepts of pointers and dynamic memory allocation 
//using allocate, deallocate to rewrite the following code 
...
int main()
{
    ...
    float *grades = new float[num_subjects];  // pointer used to refer to grades
    //dynamic array of floats is allocated using new. size is determined by num_subjects
    for (int i = 0; i < num_subjects; ++i)
    {
    ...
    //Admissions* student = new Admissions(name, age);
    ...
    //delete student;
    delete[] grades; // Array is properly deallocated using delete[] to prevent memory leaks
    return 0;
}
